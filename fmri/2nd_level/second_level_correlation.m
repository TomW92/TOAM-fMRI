function second_level_correlation(subj, myCorr, onset_type )
% second_level_acc_cat_cond - calculates teh second level analysis 
%  - GLM and behavioral correlation
% Syntax: second_level_acc_cat_cond(subj, sess, behavior_name, myClassic,myContrast,myMix )
%
% Inputs:
% subj - string of subject number
% sess - string of session number
% behavior_name - string of name of behavior dataset (without file ending)
% myClassic - DEPRICATED! boolean if classic analysis is performed. 
% myContrast - string of contrast performed on first level (naming)
% myMix - DEPRECATED
%
...
%% Setup
analysis_name=[myCorr,'_',onset_type];%,
%L1_p='l1_noMask_08_thrs_s6wr';
...
rfxDir = fullfile(config.data.bold,'RFX_Correlations',['RFX_',L1_p,'_',analysis_name]);
% Creation of directory for the rfx
if exist(rfxDir,'dir')
    rmdir(rfxDir,'s')
end
