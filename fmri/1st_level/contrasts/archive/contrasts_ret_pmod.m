function [conds, contrast, convecs, onsets, durations, pmods] = contrasts_ret_pmod(behavior)
...
convecs  = 0;
%conds = {'faceRecog';'ret'};
conds = unique(behavior.trial_type);
onsets=cell(1,length(conds));
durations=cell(1,length(conds));
pmods = cell(1,length(conds));
